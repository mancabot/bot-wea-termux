...
const { help } = require('./mcbot/help')
const { rules } = require('./mcbot/rules')
const { bahasa } = require('./mcbot/bahasa')
const { donasi } = require('./mcbot/donasi')
const { limits } = require('./mcbot/limit')
...
async function starts() {
	...
	fs.existsSync('./mcbot.json') && client.loadAuthInfo('./mcbot.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./mcbot.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const Mc-Bot = ["6283815956151@s.whatsapp.net"]
			const mcbot = mek.message.conversation
     ...
     const bayarLimit = (sender, amount) => {
        ...
        const limitAdd = (sender) => {
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                const check = await mek.inviteInfo(mcbot);
                if (!check) {
                    return
                } else {
...
break
		...
		case 'daftar':
			...
			case 'menu':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, Mc-Bot, replySet)
					}, 0)
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limits.limitend(pushname2))
				    ...
				    break 
				case 'owner':
                 client.sendMessage(from, {displayname: "mc-bot", vcard: vcard}, MessageType.contact, { quoted: mek})
...
// only grup fitur anime
              case 'anime':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					reply(mess.wait)
					anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=mc`, {method: 'get'})
					sasu = JSON.parse(JSON.stringify(anu));
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
					client.sendMessage(from, nye, image, { caption: 'mc waifu owner', quoted: mek })
					await limitAdd(sender) 
					break
					case 'rikka':
				if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				if (!isUser) return reply(mess.only.userB)
				reply(mess.wait)
				 data = fs.readFileSync('./mcbot/drak.js');
		    ...
		    case 'map':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Subscribe:https://youtube.com/channel/UCal0HWOurq6GIF4_z0N2B6Q'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                ...
                bufferddd = await getBuffer('https://raw.githubusercontent.com/manca-bot/bot-wea-termux/main/src/glitchtext.png')
                 reply(mess.wait)
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[MC-BOT]','red'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
